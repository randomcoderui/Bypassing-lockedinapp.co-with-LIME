...
## UPDATE AGAIN: ok i finally resolved the issues after 3 hours, and ima clarify about LIME V5, LIME will still update, BUT it will be 90% minor tweaks, optimization, or QOL

## LIME is compatiable with magisk and apatch (with a dedicated module)
...
this project isnt only ment to bypass lockedin, but expose every hidden security flag and expose everything inside the decompiled apk. and ultimately, make it so impossible to patch LIME that they break one of their rules. well it isnt an official rule, its just 2 FUNDEMENTAL rules. 1, they cant have false alarms, if schools get false alarms, its going to defeat the credability of lockedin. 2, they cant make their app escalate privilages, if they do that, then they would have to behave as a rootkit malware, which google will immediately smite the app off the play store. now if they try to move server side, well theres one flaw, servers are dumb, they cant tell if a phone died, lost wifi, or a bypass is used. the server will most likely flag it as a bypass, which would break rule 1. and also, we can easily fake a shutdown by broadcasting a shutdown message, as long as we are rooted, we can do almost anything. and obviously lockedin is bound by android's security and SELinux policies. and breaking that security model will result in google absolutely smiting the app off the google play store. they are fundementally trapped. anyways ive explained the 2 fundemental rules. theres only so much patches and updates they can do before they run out. and as ive said, this project isnt aimed to JUST bypass lockedin, but expose every single flaw in this app. and if lockedin tries to fight back, i have a list of anti-patches ready to go. good. luck.

## Estimated completion & misc.

now project breakout and LIME are being updated continously, i spend like 2-3 hours on this almost every day, and it might take a pretty long time to fully finish LIME, if i were to give an estimate, LIME would probably be finished in 3 weeks or so. but even then, theres going to be bugs and stablility issues. (UPDATE: i just encountered a pretty major stability issue where it would trigger androids rescue party, fortunately i was able to recover so to prevent further stability issues, i might stop LIME at v6!)so if your asking what version would be the final one, i would say v7+. LIME isnt perfect, and it might never be fully 100% effective, but its better than a standard, one shot bash script, or manually disabling the components. LIME is almost pretty straight forward, kill lockedin, and prevent it from communicating. i really dont add other stuff like xml injection, and even if i did, it would get removed. and about root detection, you CAN spoof some properties like buildprops. but unfortunately for me, lockedin will treat even a single detection as a bypass. so after i finish air tighting LIME's watchdog, i will try to focus on preventing lockedin from knowing we are rooted. i mean LIME already tackles some of it by disabling MainActivity, securityservice cant start, and the firewall prevents lockedin from sending its data to the servers. but as of 8/15 right now, im just mainly focusing on reinforcing LIME's watchdog, after that, i will TRY to buff the firewall without accidently eviscerating the phones internet, also ive said that LIME is only tested on android 14, now i DO have 2 androids, but one of them is carrier locked and the other one's bootloader is permenantly locked. so i cant really test LIME other than on android 14. so if you are on another android version like android 13 or 12, you can try to use LIME but LIME may not fully work. BUT i can say this, if you are under android 14, LIME may work or it might work even better, android 15 also might work but i might need to tweak LIME. anything above android 16 will fail. the reason why LIME is incompatiable with android 16 is that android 16 got more stricter with chcon and iptables, and it will lead to some problems, because android 16 moved away from iptables and switched to eBPF, well not completely but mostly. and chcon is restricted by SELinux since on android 16, SELinux got a bit stricter with rooted users. and chcon will most likely return an AVC denial. this dosent mean its totally impossible for LIME to function, but id have to rewrite portions of the script.
