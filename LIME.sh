...
# Clean up any leftover policy rules permanently to keep system stable
ip rule del table 99 2>/dev/null
ip -6 rule del table 99 2>/dev/null
...
unified_watchdog() {
  ...
  while true; do
    ...
    # Firewall loop - Runs 24/7 independently
    if [ -n \"\$TARGET_UID\" ] && [ \"\$TARGET_UID\" -gt 0 ]; then
        iptables -C OUTPUT -m owner --uid-owner \"\$TARGET_UID\" -j DROP 2>/dev/null || \
        iptables -I OUTPUT 1 -m owner --uid-owner \"\$TARGET_UID\" -j DROP 2>/dev/null
